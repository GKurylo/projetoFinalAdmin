...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file']) && $id > 0) {
    ...
    if (move_uploaded_file($fileTmp, $filePath)) {
        $sql = $conn->prepare("INSERT INTO noticias-imagem (album_id, nome_arquivo) VALUES (?, ?)");
...
if (isset($_GET["delete"])) {
    ...
    $sql = $conn->prepare("SELECT nome_arquivo FROM noticias-imagem WHERE id=?");
    $sql->execute([$imgId]);
    $img = $sql->fetch();
    ...
    $sql = $conn->prepare("DELETE FROM noticias-imagem WHERE id=?");
    $sql->execute([$imgId]);
...
<body>
    ...
    <?php if ($listar): ?>
        ...
        <div>
            <?php
            $sql = $conn->prepare("SELECT * FROM noticias_imagem WHERE album_id=?");
            $sql->execute([$id]);
            while ($img = $sql->fetch()):
            ?>
                <div class="img-box">
